...
1. 感情分類：`"sentence -> sentiment: bool"` 例えば肯定的な場合`True`を返す
1. 要約生成：`"document -> summary"`

複数の入力/出力フィールドと型を持つシグネチャも定義可能です：

1. 検索拡張型質問応答：`"context: list[str], question: str -> answer: str"`
1. 推論を伴う多肢選択式質問応答：`"question, choices: list[str] -> reasoning: str, selection: int"`
...
1. `dspy.InputField` の `desc` キーワード引数として入力フィールドの性質に関するヒントを提供する
1. `dspy.OutputField` の `desc` キーワード引数として出力フィールドに対する制約条件を指定する
...
1. **型付けモジュール型**：`list[str]`、`dict[str, int]`、`Optional[float]`、`Union[str, int]` など
1. **カスタム型**：ユーザーコード内で定義された型
1. **ドット表記**：適切な設定を行うことで、入れ子構造の型を表現可能
1. **特殊データ型**：`dspy.Image`、`dspy.History` などの専用データ型
...
複数の署名を組み合わせてより大きな[DSPyモジュール](../modules/)を構築し、さらにこれらのモジュールを[最適化されたプロンプト](../../optimization/optimizers/)やファインチューニング用にコンパイルすることが可能です。
